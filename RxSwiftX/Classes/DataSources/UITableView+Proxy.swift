...
public typealias UITableViewProxy = UITableViewDataSource & UITableViewDelegate

public extension Reactive where Base: UITableView {
    
    func items<Proxy: RxTableViewDataSourceType & UITableViewProxy, O: ObservableType>(proxy: Proxy)
        -> (_ source: O)
        -> Disposable
        where Proxy.Element == O.Element
    {
        return { source in
            ...
            return source.subscribeTableViewProxy(
                ...
                proxy: proxy,
                retainDelegate: true
            ) { [weak tableView = self.base] (event) in
...
fileprivate extension ObservableType {
    ...
    func subscribeTableViewProxy(
        _ tableView: UITableView,
        proxy: UITableViewProxy,
        retainDelegate: Bool,
